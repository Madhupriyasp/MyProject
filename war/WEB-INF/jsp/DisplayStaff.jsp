...
<body>
  <div class="jumbotron">
<img src="kcgimages/logo_text.gif" border="0" alt="logo"style="position: relative;
top:-50px;left:400px;height:100px" />
...
<div class="container" style="top:-100px;left:140px;position:absolute" >  

<form method="post" action="/update" name="updateform"
				id="updateform" onclick="validate()">
  ...
  if(request.getAttribute("mondaysList") != null){
		...
		List<Mondays> monday = 
                (List<Mondays>)request.getAttribute("mondaysList");
	
		%>
      
  <div class="table-responsive"style="left:200px;top:300px;position:absolute;width:1000px">
  		<h4 align="center">Time Table </h4>
  
    <p><em>Staff Name: ${staffname}</em><br><br></p>
...
for(Mondays mon : monday){
	...
	<%	
		...
		List<Tuesdays> tues_datas = 
                (List<Tuesdays>)request.getAttribute("tuesdaysList");
		...
		%>
			...
			value="<%=KeyFactory.keyToString(data.getTkey()) %>" /> 
		       ...
		       <td>Tuesday</td>
		<td><input type="text" class=" textw" name="tuesPeriod1" value="<%=data.getPeriod1() %>"
										required />
	...
	<%	
		...
		List<Wednesdays> wed_datas = 
                (List<Wednesdays>)request.getAttribute("wednesdaysList");
		...
		<input type="hidden" name="wkey" id="key" 
			value="<%=KeyFactory.keyToString(data.getWkey()) %>" /> 
				<tr class="success">      
		  				     ...
		  				     <td>Wednesday</td>
								<td><input type="text" class=" textw" name="wedPeriod1" value="<%=data.getPeriod1()%>"
										required />
	...
	<%	
		...
		List<Thursdays> thurs_datas = 
                (List<Thursdays>)request.getAttribute("thursdaysList");
		...
		<input type="hidden" name="thkey" id="key" 
			value="<%=KeyFactory.keyToString(data.getThkey()) %>" /> 
				<tr class="success">      
		
		       <td>Thursday</td>
		<td><input type="text" class=" textw" name="hursPeriod1"
	...
	<%	
		...
		List<Fridays> friday_datas = 
                (List<Fridays>)request.getAttribute("fridaysList");
		...
		<input type="hidden" name="fkey" id="key" 
			value="<%=KeyFactory.keyToString(data.getFkey()) %>" /> 
				<tr class="success">      
		
		       <td>Friday</td>
		<td><input type="text" class=" textw" name="friPeriod1" value="<%=data.getPeriod1() %>"
										required />
    ...
    <a href = "/admin">
  <input type="button" class="text " name="submit" id="submit"
				 value="Back"></a>
